...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        ssize_t sent_bytes = send(cli_socket, cmd_buff, strlen(cmd_buff) + 1, 0);
        if (sent_bytes < 0) {
            perror("[CLIENT] Error sending command");
        ...
        while (1) {
            ...
            if (is_eof) rsp_buff[recv_size - 1] = '\0'; // Remove EOF from output

            printf("%s\n", rsp_buff);
            fflush(stdout);

            if (is_eof) {
                printf("[CLIENT] EOF received, breaking out of receive loop\n");
                break;
            }
        }

        if (strcmp(cmd_buff, "exit") == 0) {
            printf("[CLIENT] Exiting client...\n");
            break;
        }
    }
