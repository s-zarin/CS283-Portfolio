...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if (build_cmd_list(io_buff, &cmd_list) != OK) {
            send_message_string(cli_socket, "Command parse error\n");
            continue;
        }
        ...
        int result = rsh_execute_pipeline(cli_socket, &cmd_list);
        printf("[SERVER] Finished executing with status: %d\n", result);
        ...
        printf("[DEBUG] Sending FINAL EOF to ensure client knows we're done.\n");
        send_message_eof(cli_socket);
        //printf("[SERVER] EOF sent\n");
    }
...
int send_message_eof(int cli_socket){
    ...
    printf("[SERVER] Sending EOF to client...\n");

    sent_len = send(cli_socket, &RDSH_EOF_CHAR, send_len, 0);

    if (sent_len != send_len){
        perror("[SERVER] Error sending EOF");
        return ERR_RDSH_COMMUNICATION;
    }

    printf("[SERVER] EOF sent to client (sent %d bytes)\n", sent_len);
    return OK;
}
...
int send_message_string(int cli_socket, char *buff) {
    if (!buff || cli_socket < 0) {
        fprintf(stderr, "[ERROR] Invalid buffer or socket in send_message_string()\n");
        return ERR_RDSH_COMMUNICATION;
    }

    int msg_len = strlen(buff);

    printf("[DEBUG] Preparing to send message: \"%s\" (size: %d bytes)\n", buff, msg_len);
    ...
    if (sent_len != msg_len) {
        perror("[ERROR] send() failed in send_message_string");
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    fprintf(stderr, "[SERVER] Setting up pipeline for %d commands\n", clist->num);
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] < 0) {
            ...
            fprintf(stderr, "[SERVER] Executing: %s\n", clist->commands[i].argv[0]);
    ...
    printf("[SERVER] Command finished with status: %d\n", last_status);
    return last_status;
}
