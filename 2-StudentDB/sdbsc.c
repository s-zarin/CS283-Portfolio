...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t existing_student = {0}; // Ensure it's zero-initialized
    if (read(fd, &existing_student, STUDENT_RECORD_SIZE) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    // Check if the whole struct is empty
    ...
    // Seek back and write new student record
    if (lseek(fd, offset, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
...
{
    // Locate student using get_student()
    student_t student;
    ...
    // Overwrite with EMPTY_STUDENT_RECORD
    off_t offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
...
int print_db(int fd)
{
    ...
    // Read through the file and count valid students
    student_t student;
    int record_count = 0;
    int first_valid_student = 1; // Flag to print header only once
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        ...
        if (first_valid_student) {
            ...
            first_valid_student = 0;
        }

        // Convert GPA to float and print student info
    ...
    // Step 3: Handle empty database case
    if (record_count == 0) {
        printf(M_DB_EMPTY);
    }
...
{
    // Step 1: Open the temporary database file
    ...
    // Step 2: Seek to the beginning of the original database
    if (lseek(fd, 0, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        close(tmp_fd);
        return ERR_DB_FILE;
    ...
    // Step 3: Read all valid students and write them to the temp file
    student_t student;
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
            continue; // Skip empty records
        ...
        // Write to temp file
        if (write(tmp_fd, &student, STUDENT_RECORD_SIZE) == -1) {
            printf(M_ERR_DB_WRITE);
            close(tmp_fd);
            return ERR_DB_FILE;
        }
    ...
    // Step 4: Close the original database file
    close(fd);

    // Step 5: Replace the old database with the compressed one
    ...
    // Step 6: Return new file descriptor for compressed database
